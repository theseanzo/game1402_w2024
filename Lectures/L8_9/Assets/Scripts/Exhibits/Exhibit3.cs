...
public class Exhibit3 : Exhibit
{
    ...
    float _score = 0; //every time we start the game we will restart the _score
    float _timeElapsed = 0; //the amount of time that has gone on in our game
    int _scoreIncrease = 1; //this could be based on the target but for now...just increase by 1
    #endregion
    ...
    public void StartGame() //when we start the game, we start spawning
    {
        if (!_gameStarted)
        {
            ...
            {
                spawner.StartSpawn();
                spawner.targetHit.AddListener(() => _score += _scoreIncrease); //when our target is hit we register an anonymous function that adds to the _score
            }
        }
	}
    public void StopGame()
    {
        _timeElapsed = gameDuration; //this is public because we may want to call it from multiple places
        _gameStarted = false;
        foreach(WaveSpawner spawner in waveSpawners)
        {
    ...
    private void FixedUpdate()
    {
        ...
        float timeRemaining = gameDuration - _timeElapsed; //display how long the game has been going
